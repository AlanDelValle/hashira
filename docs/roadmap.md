...
- [~] **10.1 Self-hosting: containers, backups, an upgrade path.** A production image and a
  ...
  **10.1b is done:** the image, the compose file, the §13 footer, nightly backups and
  ...
  table. **10.1c is published:** `v0.10.0` is tagged and
  `ghcr.io/alandelvalle/hashira` carries it and `latest`, built for `linux/amd64` and
  `linux/arm64` and pullable by anybody. What is left of the whole sub-phase is the one thing
  that cannot be written down — the machine it runs on, and a domain resolving to it before the
  first `up`, because that is what a certificate is issued against.
